...
class DenseModel(Backbone):
    ...
    def set_freeze(self, block_select, opt=None, loss=None, metrics=None):
        backbone_id = 0
        conv_layer_idx = [(idx, layer.name, layer.count_params()) for idx, layer in enumerate(self.model.layers[backbone_id].layers) if isinstance(layer, Conv2D)]
        if block_select == 0:
            for layer in self.model.layers[backbone_id].layers:
            ...
            if block_select > len(conv_layer_idx):
                ...
                for idx, layer in enumerate(self.model.layers[backbone_id].layers):
